...
TDG.Sfx = {
    manage: function (type, mode, game, button, label) {
        switch (mode) {
            case 'init': {
                TDG.Storage.initUnset('TDG-' + type, true);
                TDG.Sfx.status = TDG.Sfx.status || [];
                TDG.Sfx.status[type] = TDG.Storage.get('TDG-' + type);
                if (type == 'sound') {
                    TDG.Sfx.sounds = [];
                    TDG.Sfx.sounds['click'] = game.sound.add('sound-click');
                }
                else { // music
                    if (!TDG.Sfx.music || !TDG.Sfx.music.isPlaying) {
                        TDG.Sfx.music = game.sound.add('music-theme');
                        TDG.Sfx.music.volume = 0.5;
                    }
                }
                break;
            }
            case 'on': {
                TDG.Sfx.status[type] = true;
                break;
            }
            case 'off': {
                TDG.Sfx.status[type] = false;
                break;
            }
            case 'switch': {
                TDG.Sfx.status[type] = !TDG.Sfx.status[type];
                break;
            }
            default: { }
        }
        TDG.Sfx.update(type, button, label);

        if (type == 'music' && TDG.Sfx.music) {
            if (TDG.Sfx.status['music']) {
                if (!TDG.Sfx.music.isPlaying) {
                    TDG.Sfx.music.play({ loop: true });
                }
            }
            else {
                TDG.Sfx.music.stop();
        ...
        TDG.Storage.set('TDG-' + type, TDG.Sfx.status[type]);
    },
    play: function (audio) {
        if (audio == 'music') {
            if (TDG.Sfx.status['music'] && TDG.Sfx.music && !TDG.Sfx.music.isPlaying) {
                TDG.Sfx.music.play({ loop: true });
            }
        }
        else { // sound
            if (TDG.Sfx.status['sound'] && TDG.Sfx.sounds && TDG.Sfx.sounds[audio]) {
                TDG.Sfx.sounds[audio].play();
            }
        }
    },
    update: function (type, button, label) {
        if (button) {
            if (TDG.Sfx.status[type]) {
        ...
        if (label) {
            if (TDG.Sfx.status[type]) {
                label.setText(TDG.Lang.text[TDG.Lang.current][type + '-on']);
            }
            else {
                label.setText(TDG.Lang.text[TDG.Lang.current][type + '-off']);
            }
        }
    }
};
TDG.fadeOutIn = function (passedCallback, context) {
...
TDG.fadeOutScene = function (sceneName, context) {
    context.cameras.main.fadeOut(250);
    context.time.addEvent({
        delay: 250,
        callback: function () {
...
TDG.Lang = {
    ...
    updateLanguage: function (lang) {
        ...
        var qs = TDG.Lang.parseQueryString(query);
        if (qs && qs['lang']) {
            console.log('LANG: ' + qs['lang']);
            TDG.Lang.current = qs['lang'];
        } else {
            if (lang) {
                TDG.Lang.current = lang;
            }
            else {
                TDG.Lang.current = navigator.language;
            }
        }
        if (TDG.Lang.options.indexOf(TDG.Lang.current) == -1) {
            TDG.Lang.current = 'en';
        }
    },
    text: {
